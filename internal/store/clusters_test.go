...
func TestListDisplayClusterSummariesPrefersLatestRawRun(t *testing.T) {
	...
	if len(activeDisplay) != 1 || activeDisplay[0].ID != 700 || activeDisplay[0].Source != ClusterSourceDurable {
		t.Fatalf("active display clusters should prefer durable clusters, got %#v", activeDisplay)
	}
	...
	if len(hiddenByMinSize) != 0 {
		t.Fatalf("active display min-size should count durable active members, got %#v", hiddenByMinSize)
	}
	...
	if len(display) != 1 || display[0].ID != 700 || display[0].Source != ClusterSourceDurable {
		t.Fatalf("display clusters should prefer durable groups, got %#v", display)
	}
